...
#my $filename = 'atlas_dr3_ecdfs_matches_fusion_z.csv';
my $filename = 'atlas_radio_specz.asc';
...
#     if (index($row,'cid') != -1) {next};
     if (index($row,'SID') != -1) {next};
     
#     my @array=split(',',$row);
     my @array=split(' ',$row);
 
# if Sint = -999 then skip

#     if($array[2]== -999) {next};
     
#     my $Sint_mJy=$array[2]*1;  # elais and ecdfs
#     my $Redshift=$array[7]*1; #elais
#     my $Redshift=$array[9]*1; #ecdfs
     
# for atlas_radio_specz

   my $Sint_mJy=$array[3]*1;
   my $Redshift=$array[6]*1;
